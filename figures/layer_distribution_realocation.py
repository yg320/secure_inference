...
old = []
for layer_name in x:
    new.append(get_block_spec_num_relus({layer_name: x[layer_name]}, params) )
    old.append(get_block_spec_num_relus({layer_name: np.ones_like(x[layer_name])}, params))

bars = plt.bar(range(len(new)), old, color="#3399e6")
for bar in bars:
    bar.set_edgecolor("black")
    bar.set_linewidth(1.2)

plt.subplot(212)
bars = plt.bar(range(len(new)), new, color="#3399e6")
for bar in bars:
    bar.set_edgecolor("black")
    bar.set_linewidth(1.2)
