...
class MultipleChoiceKnapsack:
    def __init__(self, params, cost_type, division, ratio, seed, num_channels, channel_distortion_path, shuffle, target_snr):
        self.params = params
    ...
    def prepare_matrices(self):
        ...
        for channel_order in self.channel_orders:
            ...
            if self.target_snr is not None:
                noise = np.sqrt(1/self.target_snr) * np.random.normal(loc=0, scale=P.std(), size=P.shape)
                P = P + noise
    ...
    def get_optimal_block_sizes(self):
        ...
        block_size_spec, total_noise = self.convert_dp_arg_to_block_size_spec(dp_arg, Ws, block_size_tracker)

        return block_size_spec, total_noise
    ...
    def convert_dp_arg_to_block_size_spec(self, dp_arg, Ws, block_size_tracker):
        ...
        total_noise = 0
        block_size_to_index = {tuple(block_size): index for index, block_size in enumerate(self.block_sizes)}

        for channel_order in tqdm(list(reversed(range(num_channels)))):
            arg = dp_arg[channel_order][column]
            channel_cost = Ws[channel_order, arg]
            column -= channel_cost
            block_sizes.append(block_size_tracker[channel_order, arg])
            if self.target_snr:
                layer_name = self.channel_order_to_layer[channel_order]
                channel_index = self.channel_order_to_channel[channel_order]
                P = self.layer_name_to_noise[layer_name][channel_index]
                t = block_size_to_index[tuple(block_size_tracker[channel_order, arg])]
                total_noise += P[t]
        block_sizes = np.array(block_sizes[::-1])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--block_size_spec_file_name', type=str, default="/home/yakir/Data2/assets_v4/distortions/ade_20k_256x256/MobileNetV2/noisy_blocks/0.pickle")
    ...
    parser.add_argument('--target_snr', type=float, default=10)
    ...
    for i in range(100):
        seed = i
        block_size_spec_file_name = f"/home/yakir/Data2/assets_v4/distortions/ade_20k_256x256/MobileNetV2/noisy_blocks/{i}.pickle"
        mck = MultipleChoiceKnapsack(params, args.cost_type, args.division, args.ratio, seed, args.num_channels, args.channel_distortion_path, args.shuffle, args.target_snr)
        block_size_spec, total_noise = mck.get_optimal_block_sizes()

        if args.target_snr:
            with open(args.block_size_spec_file_name, "wb") as f:
                pickle.dump(obj=(block_size_spec, total_noise), file=f)
        else:
            with open(args.block_size_spec_file_name, "wb") as f:
