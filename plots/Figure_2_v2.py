...
images = [[activation, bReLU_1x1], [ activation, bReLU_3x3],[ activation, bReLU_2x3]]
for i in range(3):
    for j in range(2):
        if j == 0 and i in [0, 2]:
            continue
        ax = fig.add_subplot(3, 2, 2*i+j+1, aspect='equal')

        if i == 1 and j == 0:
            plt.title("Input activation", fontsize=13)
        if i == 0 and j == 1:
            plt.title("Output activation 1x1", fontsize=13)
        if i == 1 and j == 1:
            plt.title("Output activation 3x3", fontsize=13)
        if i == 2 and j == 1:
