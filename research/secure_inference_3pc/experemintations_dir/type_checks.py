...
A_numpy = A_.numpy()#.astype(np.float64)
...
print(time.time() - t0)
#
# A_float = A.to(torch.float64)
# B_float = B.to(torch.float64)
#
# t0 = time.time()
# C_float = torch.conv2d(A_float, B_float, bias=None, stride=1, padding=1, dilation=1, groups=1)
# print(time.time() - t0)
# C__ = post_conv(None, C_, b, c, h, w )
# print(time.time() - t0)
#
# t0 = time.time()
# C = torch.conv2d(A, B, bias=None, stride=1, padding=1, dilation=1, groups=1)
