...
# for i in range(57):
#     layer_name = params.LAYER_NAMES[i]
#     if not os.path.exists(f"/home/yakir/Data2/assets_v3/deformations/coco_stuff164k/ResNetV1c/channel_knapsack/{layer_name}_reduction_to_block_sizes.npy"):
#         print(i)
...
# assert False, "should we take different statistics?, which is linear with loss"
...
#     for relu_count in range(num_relus):
#         indices = np.maximum((relu_count - W), 0)
#         tmp = dp[channel - 1, indices] + P[channel]
#         dp_arg[channel, relu_count] = tmp.argmax()
#         dp[channel, relu_count] = tmp[dp_arg[channel, relu_count]]
...
# for channel in tqdm(range(1, channels)):
#
#     r = dp[channel - 1, all_indices].reshape(orig_shape) + P[channel]
#     dp_arg[channel] = np.argmax(r, axis=1)
#     dp[channel] = r[arange, dp_arg[channel]]
#
...
relus_count = np.floor(np.arange(0, 1.001, 0.001) * (num_relus - 1)).astype(np.int32)
...
np.save(file=f"/home/yakir/Data2/assets_v3/deformations/coco_stuff164k/ResNetV1c/channel_knapsack/{layer_name}_reduction_to_block_sizes.npy", arr=all_block_sizes)
