...
def run_model_block_mobilenet(model, activation, block_name):
    if block_name == "conv1":
        ...
        res_layer_name, block_name = block_name.split("_")
        layer = getattr(model.backbone, res_layer_name)
        activation = layer[block_name](activation)
    return activation

def get_layer_mobilenet(model, layer_name):
    if "decode" in layer_name:
        ...
        return layer[inverted_residual_block].conv[conv_module].activate


def set_layer_mobilenet(model, layer_name, block_relu):
    if "decode" in layer_name:
        if layer_name == "decode_0":
        ...
        layer_name, inverted_residual_block, conv_module = layer_name.split("_")
        layer = getattr(model.backbone, layer_name)
        layer[inverted_residual_block].conv[conv_module].activate = block_relu
